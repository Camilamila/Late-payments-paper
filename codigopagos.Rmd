---
title: "The cost of late payments in public procurement (R script)"
output: html_document
author: "Camila Salazar"
---
<base target="_blank"/>
This script was used for the paper "The cost of late payments in public procurement", presented in the Open Data Research Symposium 2018. The datasets used for this analysis can be downloaded from this link:  [Datasets](https://drive.google.com/open?id=1uoksGuJ0YTnnkma58FmJDMyPZjNSwG7u)
...
```{r eval=F, echo=T, message=FALSE, warning=FALSE, tidy=TRUE}
library(dplyr)
library(lubridate)
library(ggplot2) 
library(readxl)
...
#Import files
base2<-read_csv("originaldataset.csv")
...
base2[,35:48] <- lapply(base2[,35:48], as.numeric)

base2$obl_entidad_descripcion<- gsub("\uFFFD", "" , base2$obl_entidad_descripcion, fixed=T)
base2$obl_grupo_descripcion<- gsub("\uFFFD", "" , base2$obl_grupo_descripcion, fixed=T)
...
ggplot(etapas_detalle, aes(fill=etapa, y=duracion, x=obl_anho_obligacion)) +
  ...
  scale_fill_manual(" ", 
                    values = c("Invoice approval"="#559171", "TR creation"="#b5c62d", 
                               "TR approval"="#6bac8f", "Transfer to Treasury"="#FCA32B",
...
ggplot(etapas3, aes(fill=stage, y=duration, x=reorder(obl_entidad_descripcion, duration))) +
  ...
  labs(title="Average payment duration by stage",
       subtitle="top 10 entities with higher spending",
       y="Total duration (days)",
       x= "",
       caption = "Based on data from datos.hacienda.gov.py")
...
proveedores2<-base_limpia%>%
  ...
  summarise(monto=sum(obl_monto_obligado),
            contratos=n_distinct(contr_codigo_contratacion),
...
datos$mes_aniof<-as.Date(as.yearmon(datos$mes_aniof))
datos$mes_aniop<-as.Date(as.yearmon(datos$mes_aniop))
...
surv<-datos%>%
  ...
  filter(!is.na(size))
surv$obl_nivel_descripcion<- gsub("\uFFFD", "" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("14-CONTRALORA GENERAL DE LA REPBLICA", "Legislative Branch" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("11-PODER LEGISLATIVO", "Legislative Branch" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("15-DEFENSORA DEL PUEBLO", "Legislative Branch" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("15-OTROS ORGANISMOS DEL ESTADO", "Other" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("23-ENTES AUTNOMOS Y AUTRQUICOS", "Other" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("27-ENTIDADES FINANCIERAS OFICIALES", "Other" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("28-UNIVERSIDADES NACIONALES", "Universities" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("12-PODER EJECUTIVO", "Executive Branch" , surv$obl_nivel_descripcion, fixed=T)
surv$obl_nivel_descripcion<- gsub("13-PODER JUDICIAL", "Judiciary Branch" , surv$obl_nivel_descripcion, fixed=T)
...
ggsurvplot(fit1, data = surv, pval = F,
           fun = "pct",
           palette=c("#559171",
                     "#b5c62d",
                     "#FCA32B"),
...
ggsurvplot(fit3, data = surv, pval = F,
           fun = "pct",
...
costo_gdp17<-costos%>%
  ...
  mutate(costo_dolar=(monto/5618.933)/1000000,
         ...
         perc_gdp=monto/GDP_2017*100)

#PIB percentage
gdp <- read_excel("GDP.xlsx")
costo_total_anio<-costos%>%
  left_join(gdp, by=c("obl_anho_obligacion"="anio"))%>%
  group_by(obl_anho_obligacion, gdp)%>%
  summarise(costonominal=sum(costo_total_nominal))%>%
  mutate(porc_gdp=round((costonominal/gdp*100), 3))

ggplot(costo_total_anio, aes(x=obl_anho_obligacion, y=porc_gdp)) + 
  geom_line() +
  theme(legend.title = element_blank())+
  theme_Publication()+
  scale_y_continuous(labels=comma)+
  labs(title="Total cost as a percentage of GDP",
       subtitle="Datasets A and B",
       y="% of GDP",
       x= "Year",
       caption = "Based on data from datos.hacienda.gov.py")+
  scale_x_continuous(breaks=seq(2011,2017,1))
...
year_cost<-costos%>%
  ...
  summarise('Total cost'=(sum(costo_total)/1000000)/5618.933)%>%
...
ggplot(costo_etapa, aes(x=stage, y=costo_dolar, fill=costo)) +
  geom_bar( stat="identity")+theme_Publication() + coord_flip()+
  labs(title= "Cost by stages",
       subtitle="Dataset A and B",
       y="Millions of dollars",
       x= "",
       caption = "Based on data from datos.hacienda.gov.py")+
